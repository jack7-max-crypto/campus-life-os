...
export default function AcademicsPage() {
  ...
  const gpaOverview = useMemo(() => {
    ...
    const whatIfGrades = {
      ...projectedGrades,
      [whatIfCourseId]:
        courses.find((course) => course.id === whatIfCourseId) && Number.isFinite(Number(whatIfGrade))
          ? parsedWhatIf
          : projectedGrades[whatIfCourseId] ?? 0,
    };
    const whatIfProjectedGpa = computeWeightedGpa(courses, whatIfGrades, gpaScaleId);

    const baselineProjectedGpa = projectedGpa;
    const impacts = courses.map((course) => {
      const baseline = projectedGrades[course.id] ?? 0;
      const plusTenGrades = { ...projectedGrades, [course.id]: clampPercentage(baseline + 10) };
      const minusTenGrades = { ...projectedGrades, [course.id]: clampPercentage(baseline - 10) };
      const upside = computeWeightedGpa(courses, plusTenGrades, gpaScaleId) - baselineProjectedGpa;
      const downside = baselineProjectedGpa - computeWeightedGpa(courses, minusTenGrades, gpaScaleId);
      return {
        course,
        upsideImpact: Math.max(0, upside),
        downsideImpact: Math.max(0, downside),
      };
    });
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <section>
        <Card title="GPA Overview" subtitle="4.0 scale using course credits">
          ...
          <MetricRow label="Best impact course" value={gpaOverview.bestImpactCourse?.name ?? "—"} />
          <MetricRow label="Worst impact course" value={gpaOverview.worstImpactCourse?.name ?? "—"} />
